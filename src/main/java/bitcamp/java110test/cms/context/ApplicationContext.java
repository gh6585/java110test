...
public class ApplicationContext {
    ...
    public ApplicationContext(String packageName) throws Exception{
        ...
        // 로딩된 클래스 목록을 뒤져서 @Commponent 가 붙은
        // 클래스에 대해 인스턴스를 생성하여 objPool에 보관한다.
        createInstance();

        // injectDependency() 메서드를 외부 클래스로 분리한 다음에
        // 그 객체를 실행한다.
        AutowiredAnnoationBeanPostProcessor processor =
                new AutowiredAnnoationBeanPostProcessor();
    ...
    public void findClass(File path, String packagePath) {
    ...
    private void createInstance() {
        for(Class<?> clazz : classes) {
            ...
            // => @Component 애노테이션이 붙지 않은 클래스는 객체르 생성하지 않는다.
            if(anno == null) continue;

            try {
                // 로딩된 클래스정보를 가지고 인스턴스를 생성한다.
                ...
                if(anno.value().length()>0) {
                    // => 애노테이션 value 값으로 인스턴스를 objPool에 저장한다.
                ...
                e.printStackTrace();
                System.out.println("%s 클래스는 기본 생성자가 없습니다.");
                clazz.getName();
            }
        }
    }
