...
//
//      http://localhost:8888/java110-servlet2/ex01/servlet01
//
...
public class Servlet01 implements Servlet{
    ...
    public void init(ServletConfig config) throws ServletException {
        // 서블릿 객체를 만든 후 생성자 다음으로 호출되기 떄문에
        // 이 메서드에는 서블릿이 작업할 때 사용할 때 사용할 자원을 준비하는 일을 한다.
        // 예를 들면, DB 커넥션을 준ㄴ비한다거나 외부 서버에 소켓을 연결하는 등의 작업이다.
...
//      - init() 호출
//      - service() 호출
// - 만약 서블릿 컨테이너를 종료하거나 웹 애플리케이션의 실행을 멈춘다며느
//      - 생성된 모든 서블릿들에 대해 destroy() 호출
//      - 생성된 모든 서블릿 객체를 가비지로 만든다.
//
// 주의!
// - 서블릿 객체는클랫 ㅡ당 한 개만 생성된다.
// - 요청할 때마다 생성되는 것이 아니다.
